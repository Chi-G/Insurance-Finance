                        ...
                        <div class="widget-content widget-content-area br-6">
                            ...
                            <div class="table-responsive mb-4 mt-4">
                                <table id="zero-config" class="table table-bordered table-hover dataTable table-condensed mb-4">
                                    ...
                                    <tbody>
                                        @foreach ($faqContacts as $faqContact)
                                        <tr>
                                        <td>{{ $faqContact->id }}</td>
                                        <td>{{ $faqContact->name }}</td>
                                        <td>{{ $faqContact->email }}</td>
                                        <td>{{ $faqContact->subject }}</td>
                                        <td>{{ $faqContact->message }}</td>
                                        <td>{{ $faqContact->created_at->format('d M Y') }}</td>
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <li>
                                                    <form action="{{ route('faqcontact.destroy', $faqContact->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" style="border:none; background:none;" class="warning confirm btn btn-danger delete-button" data-id="{{ $faqContact->id }}" data-toggle="tooltip" data-placement="top" title="Delete">
